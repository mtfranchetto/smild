...
BuildTask.prototype.action = function (done) {
    ...
    async.mapSeries(targets, _.bind(function (target, callback) {
        ...
        this._taskRunner.run(gulp.series(gulp.parallel(['views', 'styles', 'images', 'assets', 'js']), 'rev', 'manifest', 'post-build', function () {
        ...
        if (!this._buildHelper.isWatching())
            process.exit(0); //Exit manually due to a regression with Node 4
    }, this));
};
