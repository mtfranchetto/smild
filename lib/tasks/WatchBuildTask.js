...
WatchBuildTask.prototype.action = function () {
    ...
    this._taskRunner.run(gulp.parallel('build', 'serve', function () {
        gulp.watch([path.resolve(options.targets, target, 'bootstrapper.scss'),
                path.resolve(options.targets, 'base.scss'),
                path.resolve(process.cwd(), options.styles) + '/**/*.scss'], {maxListeners: 999},
            gulp.series('styles'));

        gulp.watch([options.views + '/**/*.html'], {maxListeners: 999}, gulp.series('views'));
    }));
};
